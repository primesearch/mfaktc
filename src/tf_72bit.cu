...
__launch_bounds__(THREADS_PER_BLOCK, 2)
    mfaktc_71(unsigned int exp, int72 k, unsigned int *k_tab, int shiftcount, int144 b, unsigned int *RES)
#else
__launch_bounds__(THREADS_PER_BLOCK, 2)
    mfaktc_71(unsigned int exp, int72 k, unsigned int *k_tab, int shiftcount, int144 b, unsigned int *RES, unsigned int *modbasecase_debug)
#endif
/*
...
a is precomputed on host ONCE. */
{
    ...
    exp72.d2 = 0;
    exp72.d1 = exp >> 23;
    exp72.d0 = (exp & 0x7FFFFF) << 1; // exp72 = 2 * exp
    ...
    k.d0 &= 0xFFFFFF;
    k.d2 += k.d1 >> 24;
    k.d1 &= 0xFFFFFF; // k = k + k_tab[index] * NUM_CLASSES
...
Precalculated here since it is the same for all steps in the following loop */
    ...
    while (exp) {
        if (exp & 0x80000000)
            square_72_144_shl(&b, a); // b = 2 * a^2 ("optional multiply by 2" in Prime 95 documentation)
        else
