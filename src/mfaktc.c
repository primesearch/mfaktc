...
see benchmarks in src/kernel_benchmarks.txt */
    ...
    if (mystuff->mode == MODE_NORMAL) {
        if ((mystuff->checkpoints == 1) &&
            (checkpoint_read(mystuff->exponent, mystuff->bit_min, mystuff->bit_max_stage, &cur_class, &factorsfound,
                             mystuff->factors_string, &(mystuff->stats.bit_level_time)) == 1)) {
                ...
                if (mystuff->mode == MODE_NORMAL) {
                    if (numfactors > 0) {
                        ...
                        for (i = 0; (i < numfactors) && (i < 10); i++) {
                            ...
                            sprintf(temp_factors_string, mystuff->factors_string[0] ? "%s,\"%s\"" : "%s\"%s\"", mystuff->factors_string,
                                    factorstring);
                    ...
                    if (mystuff->checkpoints == 1) {
                        if (numfactors > 0 ||
                            ...
                            checkpoint_write(mystuff->exponent, mystuff->bit_min, mystuff->bit_max_stage, cur_class, factorsfound,
                                             mystuff->factors_string, mystuff->stats.bit_level_time);
                        }
                    }
...
int main(int argc, char **argv)
{
    ...
    mystuff.gpu_sieve_processing_size =
        GPU_SIEVE_PROCESS_SIZE_DEFAULT * 1024; /* Default to 8K bits processed by each block in a Barrett kernel. */
...
#else
    if (mystuff.compcapa_major == 1)
        mystuff.max_shared_memory = 16384; /* assume 16kiB for CC 1.x */
    else
        mystuff.max_shared_memory = 49152; /* assume 48kiB for all other */
#endif
    if (mystuff.verbosity >= 1) {
        logprintf(&mystuff, "\nCUDA device info\n");
