...
*/
{
  ...
  {
    sprintf(buffer,"%s%u %d %d %d %s: %d %d %s %llu", NAME_NUMBERS, exp, bit_min, bit_max, NUM_CLASSES, MFAKTC_CHECKPOINT_VERSION, cur_class, num_factors, strlen(factors_string) ? factors_string : "0", bit_level_time);
    i = crc32_checksum(buffer,strlen(buffer));
    fprintf(f,"%s%u %d %d %d %s: %d %d %s %llu %08X", NAME_NUMBERS, exp, bit_min, bit_max, NUM_CLASSES, MFAKTC_CHECKPOINT_VERSION, cur_class, num_factors, strlen(factors_string) ? factors_string : "0", bit_level_time, i);
    fclose(f);
    f = NULL;
  }
}
...
*/
{
  FILE *f;
  int ret = 0, i, chksum;
  char buffer[600], buffer2[600], *ptr, filename[40];
  ...
  *num_factors = 0;
  ...
  if(ptr == buffer)
  {
    ...
    if(i < 70)
    {
      ...
      sprintf(buffer2,"%s%u %d %d %d %s: %d %d %s %llu", NAME_NUMBERS, exp, bit_min, bit_max, NUM_CLASSES, MFAKTC_CHECKPOINT_VERSION, *cur_class, *num_factors, factors_string, *bit_level_time);
      chksum= crc32_checksum(buffer2,strlen(buffer2));
      sprintf(buffer2,"%s%u %d %d %d %s: %d %d %s %llu %08X", NAME_NUMBERS, exp, bit_min, bit_max, NUM_CLASSES, MFAKTC_CHECKPOINT_VERSION, *cur_class, *num_factors, factors_string, *bit_level_time, chksum);
      if(*cur_class >= 0 && \
