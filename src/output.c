...
{
    printf("mfaktc v%s Copyright (C) 2009-2015  Oliver Weihe (o.weihe@t-online.de)\n", MFAKTC_VERSION);
...
{
    char digit[MAX_DEZ_96_STRING_LENGTH] = { 0 };
...
{
    char digit[MAX_DEZ_192_STRING_LENGTH] = { 0 };
    int digits = 0, carry = 0, i = 0;
    long long int tmp = 0;
...
#endif
    if (factorsfound) {
        sprintf(txtstring, "found %d factor%s for %s%u from 2^%2d to 2^%2d %s[mfaktc %s %s]", factorsfound, (factorsfound > 1) ? "s" : "",
                NAME_NUMBERS, mystuff->exponent, mystuff->bit_min, mystuff->bit_max_stage, partialresult ? "(partially tested) " : "",
                MFAKTC_VERSION, mystuff->stats.kernelname);
    } else {
        sprintf(txtstring, "no factor for %s%u from 2^%d to 2^%d [mfaktc %s %s]", NAME_NUMBERS, mystuff->exponent, mystuff->bit_min,
                mystuff->bit_max_stage, MFAKTC_VERSION, mystuff->stats.kernelname);
    }
#ifndef WAGSTAFF
    snprintf(
        jsonstring, sizeof(jsonstring),
        "{\"exponent\":%u, \"worktype\":\"TF\", \"status\":\"%s\", \"bitlo\":%d, \"bithi\":%d, \"rangecomplete\":%s%s, \"program\":{\"name\":\"mfaktc\", \"version\":\"%s\", \"subversion\":\"%s\"}, \"timestamp\":\"%s\"%s%s%s%s}",
        mystuff->exponent, factorsfound > 0 ? "F" : "NF", mystuff->bit_min, mystuff->bit_max_stage, partialresult ? "false" : "true",
        factorjson, MFAKTC_VERSION, mystuff->stats.kernelname, timestamp, userjson, computerjson, aidjson, osjson);
#endif
...
void print_factor(mystuff_t *mystuff, int factor_number, char *factor)
{
    ...
    if (factor_number < 10) {
        ...
        if (mystuff->mode == MODE_NORMAL) {
#ifndef MORE_CLASSES
            fprintf(resultfile, "%s%s%u has a factor: %s [TF:%d:%d%s:mfaktc %s %s]\n", UID, NAME_NUMBERS, mystuff->exponent, factor,
                    mystuff->bit_min, mystuff->bit_max_stage,
                    ((mystuff->stopafterfactor == 2) && (mystuff->stats.class_counter < 96)) ? "*" : "", MFAKTC_VERSION,
                    mystuff->stats.kernelname);
#else
            fprintf(resultfile, "%s%s%u has a factor: %s [TF:%d:%d%s:mfaktc %s %s]\n", UID, NAME_NUMBERS, mystuff->exponent, factor,
                    mystuff->bit_min, mystuff->bit_max_stage,
                    ((mystuff->stopafterfactor == 2) && (mystuff->stats.class_counter < 960)) ? "*" : "", MFAKTC_VERSION,
                    mystuff->stats.kernelname);
#endif
        }
